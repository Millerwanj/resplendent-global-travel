(function(){
  ...
  function optionLabel(plan){
    ...
    return '<article class="esim-plan"><p class="eyebrow">Resplendent eSIM</p><h3>'+escapeHtml(data)+'</h3><p>'+escapeHtml(validity)+' · '+escapeHtml(plan.scope==='global'?'Global coverage':'Destination coverage')+'</p><strong>'+money(plan.retail_price)+'</strong><a class="text-link" href="contact?service=eSIM&plan='+encodeURIComponent(plan.id)+'">Select this plan</a></article>';
  }
  ...
  function loadOffers(){
    ...
    fetch(api+'?action=offers&destination='+encodeURIComponent(destination.value),{headers:{Accept:'application/json'}})
  ...
  fetch(api+'?action=destinations',{headers:{Accept:'application/json'}})
