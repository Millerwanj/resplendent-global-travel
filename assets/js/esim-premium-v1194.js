(function(){
  ...
  var destinations=[], currentPlans=[], expanded=false;
  ...
  function renderPlans(){
    ...
    if(currentPlans.length>3){more.hidden=false;more.textContent=expanded?'Show recommended plans':'View all '+currentPlans.length+' plans';}else more.hidden=true;
  }
  function loadOffers(){
    if(!destination.value){results.innerHTML='<div class="esim-plan-placeholder"><strong>Select a destination above.</strong><span>We will show the most useful plans here.</span></div>';setStatus('Choose a destination to compare plans.');return;}
    ...
    fetch(api+'?action=offers&destination='+encodeURIComponent(destination.value),{cache:'no-store',headers:{Accept:'application/json'}})
      ...
      .then(function(payload){currentPlans=payload.offers||[];if(!currentPlans.length){results.innerHTML='<div class="esim-plan-placeholder"><strong>No plans are currently listed for this destination.</strong><span>Please choose another destination or try again shortly.</span></div>';setStatus('No plans are currently available.');more.hidden=true;return;}renderPlans();setStatus('Plans ready. Choose the option that suits your trip.','ready');})
      .catch(function(){currentPlans=[];more.hidden=true;results.innerHTML='<div class="esim-plan-placeholder"><strong>Plans are refreshing.</strong><span>Please try again in a moment.</span><button type="button" class="outline-button" data-esim-retry>Try again</button></div>';setStatus('We could not refresh plans just now.','quiet');var retry=results.querySelector('[data-esim-retry]');if(retry)retry.addEventListener('click',loadOffers);});
  }
  ...
  more.addEventListener('click',function(){expanded=!expanded;renderPlans();});
})();
